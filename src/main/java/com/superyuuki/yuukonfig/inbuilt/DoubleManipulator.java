...
public class DoubleManipulator implements Manipulator {
    ...
    public DoubleManipulator(Manipulation manipulation, Class<?> useClass, Contextual<Type> typeContextual) {
    ...
    public YamlNode serializeDefault(String[] comment) {
        return Yaml.createYamlScalarBuilder().addLine("0.0").buildPlainScalar(Arrays.asList(comment), "(Default Double)");
    }
}
